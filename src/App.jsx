...
const AJUDA_FII_DET={
  ffo:"FFO (Funds From Operations) é o \"lucro\" de um FII: o resultado operacional sem depreciação, que em fundo imobiliário distorce o número porque imóvel não se deprecia como máquina. É o que sustenta a distribuição. A tabela põe FFO e distribuição lado a lado, na mesma unidade, trimestre a trimestre. Cobertura é FFO ÷ distribuído: abaixo de 100% o fundo pagou com venda de ativo ou caixa acumulado — dura um tempo, não para sempre. A média é agregada (soma dos FFO ÷ soma do distribuído da janela), não média dos percentuais, para não dar o mesmo peso a trimestres de tamanhos diferentes. Armadilha: trimestre sem distribuição aparece como \"—\" e entra no agregado como zero — ele empurra a cobertura para CIMA, porque soma resultado sem somar pagamento. Trimestre com FFO negativo é subtraído do agregado e fica sem percentual próprio. (Não usamos \"FFO por cota\" porque exigiria o nº de cotas de cada trimestre, e FII faz emissão com frequência — dividir o FFO antigo pelas cotas de hoje viraria uma falsa curva de crescimento.)",
...
function InvestimentosTab({data,setData,currency,profileId,userId}){
  ...
  // Estimativa em R$ usa o DY LÍQUIDO quando o Worker o calculou; cai no bruto
  // só para ativo cujo dy veio do Yahoo (AU/US), onde não há retenção na fonte
  ...
  async function buscarDados(inv){
    ...
    if(real?.preco_atual){
      ...
      // DY líquido de IR (15% só sobre JCP; dividendo é isento). Vem calculado
      // do Worker, pelos proventos reais — a estimativa em R$ usa ESTE, para
      // ficar na mesma base dos proventos recebidos (G2), que também são
