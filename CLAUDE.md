...
- **Proxy/API:** Cloudflare Worker (`controlfinanceiro.leeo-parms.workers.dev`). Código versionado em `worker/worker.js` desde 25/07/2026. **CORRIGIDO em 22/09/2026:** a nota anterior dizia que o Worker "não tem deploy via git" e exigia colar no painel — **falso hoje**. `wrangler.toml` está configurado (`name="controlfinanceiro"`, `main="worker/worker.js"`, `account_id`, cron `0 21 * * *`, `keep_vars=true`) e o wrangler 4.114.0 está em devDependencies. O deploy é `./node_modules/.bin/wrangler deploy`. Exige `wrangler login` numa sessão interativa (o token expira; o Claude não consegue rodar o login, que abre navegador). `keep_vars=true` preserva os secrets no deploy. Editar `worker/worker.js` continua NÃO mudando produção sozinho — precisa do comando. Se uma mudança envolver o Worker, avisar o Leo explicitamente que ele precisa fazer esse deploy manual. Segredos (BRAPI_TOKEN, GEMINI_KEY, ANTHROPIC_KEY, CRON_SECRET, VAPID_PRIVATE_JWK) vêm de env/secrets do Cloudflare — nunca hardcodar no arquivo.
...
- **🟡 Números de IA EXIBIDOS (não gravados) — bloco separado, não corrigido.** 8 chamadas devolvem números que vão para a tela sem passar por `profiles.data`: preço-alvo de notícias (App.jsx:3708), `preco`/`pl`/`pvp`/`dy`/`roe`/`cagr_lucro`/`score` das oportunidades (:3751, :3791), indicadores do raio-X por IA (:3824), `score_carteira` e `retorno_estimado_12m` (:3859), o comparador (:3890), alocação e risco (:3902, :3909). Não contaminam dado gravado, mas são apresentados com a mesma aparência de dado apurado. **Nota sobre o comparador:** o prompt diz *"indicadores fundamentais (NÃO preço)"* — alguém teve o cuidado pela metade, excluiu o preço e manteve P/L, P/VP, DY e ROE, que são igualmente verificáveis e igualmente inventáveis. Decidir: selo de "estimativa de IA" ou remoção.
- **G5 — cobertura de FII no painel de proventos (NÃO IMPLEMENTADO, sem caminho exercido).** Desenho aprovado em 21/08/2026, adiado por falta de dado: **não existe nenhum FII na carteira** — os tipos presentes nos três perfis são `Ações` e `Renda Fixa`. Quando houver FII, mostrar no painel que parte do provento é devolução de capital, reusando `coberturaFfoFii` (calc.mjs), que é pura e desacoplada. **Restrição de desenho já decidida: SOB DEMANDA, nunca automático ao abrir a aba.** Os dados de FFO vêm de `WORKER/fii-detalhe?papel=…`, uma requisição por fundo, e não são persistidos em `profiles.data` — N fundos = N requisições por boot, a mesma classe do incidente de egress de julho. Botão explícito por ativo, ou um que carregue todos com o custo declarado na tela. Referência de valor: MXRF11 tinha cobertura agregada de 85,5% em 08/2026. Ao implementar, lembrar que isto nasce sem nenhum caminho exercido e não terá como ser verificado com dado real até existir um FII.
...
## Número gravado carrega FONTE — REGRA

**Todo dado de MERCADO em `profiles.data` carrega `fonteCotacao`: `"worker"` | `"bcb"` | `"manual"` | `"legado"`.** A trava em `salvarComRetry` (mesmo choke point da trava de base64) **remove** o que não tiver fonte reconhecida. Campos cobertos: `preco_atual`, `variacao_dia`, `dy`, `valor_dividendo`, `prox_dividendo`, `ex_dividendo`. Campos digitados pelo usuário — `precoMedio`, `quantidade`, `valorInvestido` — **não** entram: a trava é para número apurado de fonte externa.

Motivo (25/08/2026): a regra nº 4 ("números SEMPRE calculados em JS, nunca pela IA") estava sendo violada em **3 pontos**, e ninguém percebeu por meses:

| campo | como era pego | por que passou despercebido |
|---|---|---|
| `dy` | prompt sem unidade | o Leo notou porque a aritmética não fechava: "DY 0.0827%" com R$ 8,23/ano sobre R$ 9.945 |
| `valor_dividendo` | idem | nunca conferido |
| **`preco_atual`** | `"Preço de fechamento mais recente... {"preco_atual":number}"` | **INDETECTÁVEL** — R$ 18,60 e R$ 19,40 são igualmente plausíveis |

O `preco_atual` é o caso que justifica a regra: ele alimentava `valorAtual`, `lucro`, o patrimônio, a rentabilidade **e o snapshot mensal congelado por 24 meses**. Um preço alucinado não ficaria numa tela — viraria histórico.

**Vazio é informação; número alucinado não é.** Sem fonte apurada, o campo fica vazio e a tela avisa.

**ORDEM OBRIGATÓRIA — a parte perigosa do desenho:** a migração (`marcaFonteLegado`) roda ANTES da trava. Dry-run de 25/08/2026: sem ela, **22 de 22 ativos perderiam todo o dado de mercado**; com ela, 0. Há teste fixando essa dependência.

**Valores de origem duvidosa são APAGADOS, nunca convertidos.** Os 4 `dy` do BR estavam em fração (0,088 = 8,8%); multiplicar por 100 preservaria um número inventado só porque parece plausível. A distinção é a **fonte**, não o valor — o NVDA a `0,5` é percentual legítimo e sobrevive porque veio do Worker.

**⚠️ Snapshots anteriores a 25/08/2026 NÃO têm marca de origem e NÃO são auditáveis.** `historico[].ativos` guarda `{id, ticker, descricao, quantidade, valorAtual}` — nenhum campo de origem, e o preço unitário nem aparece explicitamente. Auditei em 25/08: os 9 ativos de RV têm hoje `variacao_dia` e `ultimaAtualizacao` (marca indireta do Worker), e os preços implícitos nos 6 snapshots são coerentes com a série e com o preço atual — mas isso só descarta alucinação GROSSEIRA. **Não há como confirmar nem descartar contaminação passada.** Decisão do Leo, correta: não mexer em snapshot por suspeita não confirmada.
